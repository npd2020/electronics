...
void graph(){
    ...
    double rc=20e3*154e-9; // 0.0252943
    TF1* theoryK = new TF1("K", [](double* x, double* pars){return x[0]*pars[0]/sqrt(1+pow(x[0]*pars[0],2));},-2,1.5e3,1);
    // theoryK->Draw("same");
    ...
    TF1* theoryPh = new TF1("Ph", [](double* x, double* pars){return TMath::ATan(1/(x[0]*pars[0]));},-2,1.5e3,1);
    // theoryPh->Draw("same");
