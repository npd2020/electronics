...
TGraph* ampDelta = new TGraph(0,new double[0]{},new double[0]{});
TGraph* phaseDelta = new TGraph(0,new double[0]{},new double[0]{});
      ...
      while ((ent = readdir (dir)) != NULL) {
        ...
        if((folder = opendir (Form("./%s",ent->d_name))) != NULL){
            ...
            if (dataset.size()==2){        
                ...
                TH1D *hsin = new TH1D("hsin", "hsin", graphCh1->GetN(), graphCh1->GetX()[0], graphCh1->GetX()[graphCh1->GetN()-1]);

                   //Fill the histogram with function values
                   for (Int_t i=0; i<=graphCh1->GetN(); i++){
                      hsin->SetBinContent(i+1, graphCh1->GetY()[i]);
                   }
                   hsin->Draw("same");
                   canv->cd(3);
                ...
                double freq = 13.5/graphCh1->GetXaxis()->GetXmax();
                fff->SetParameter(1,freq);
                ...
                // Data outputdata(xdata,ydata,empty,empty,0,dataset[0].n);
                // TCanvas* canv = drawgraph(outputdata, ent->d_name);
                
                // setFitStyleCanvas(canv);
