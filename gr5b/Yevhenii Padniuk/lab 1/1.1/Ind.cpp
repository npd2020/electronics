...
void Ind()
{
	std::ifstream Points("InduCpoints.txt");

	double* x = new double[20]; //Frequency(kHz)
	double* y1 = new double[20]; //Capacity(nanoFarad)
	...
	gr1 -> GetYaxis () -> SetTitle ( "C (nF)" );
	gr1 -> GetXaxis () -> SetTitle ( "#nu (kHz)" );
	gr1 -> SetLineColor(kAzure+2);
	gr1 -> Draw ( "AC *" );
	c1 -> SaveAs("Induc_Capacity.pdf");
	...
	gr2 -> Draw ( "AC *" );
	c2 -> SaveAs("Induc_Inductance.pdf");
